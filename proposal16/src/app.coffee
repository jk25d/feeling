...
class DB
  ...
  put_email: (email, uid) -> @_emails[email] = uid
  del_email: (email) -> delete @_emails[email]
  users: -> @_users
  user: (id) -> @_users[id]
  put_user: (id, data) -> @_users[id] = data
  del_user: (id) -> delete @_users[id]
  feelings: -> _feelings
  feeling: (id) -> @_feelings[id]
  put_feeling: (id, data) -> @_feelings[id] = data
  del_feeling: (id) -> delete @_feelings[id]
  

class Session
  @create: (user_id) ->
    s = new Session(user_id)
    gDB.put_session user_id, s    
  @EXPIRE_TIME: 60 * 1000
  constructor: (@user_id) ->
    @update()
  update: ->
...
class Dispatcher
  ...
  register_user: (user_id) ->
    @user_que.push new WaitItem(user_id)
  log: ->
