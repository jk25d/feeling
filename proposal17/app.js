...
(function() {
  var DB, Dispatcher, Dispatcher2, Feeling, OrderedMap, OrderedSet, ReverdSet, Session, User, UserFeelings, WaitItem, app, auto_feeling, clone, concat, config, express, feeling_seeds, gDB, gDispatcher, gFeelingContainer, len, max, merge_feelings, min, now, rand, remove_item, require_auth, u0, u1, u2, valid_session, _ref, _ref1,
  ...
  User = (function() {
    ...
    function User(name, img, email, password) {
      ...
      this.wait_time = now() - config.user_receive_wait_time;
    }

    User.prototype.has_available_feeling = function() {
      return now() - this.wait_time > config.user_receive_wait_time && gDispatcher.available_items(this.id);
    };
    ...
    User.prototype.summary = function() {
      ...
      u.has_available_feeling = this.has_available_feeling();
      return u;
    };

    User.prototype.grab_feeling = function(fid) {
      this.feelings.push_rcv(fid);
      return this.wait_time = now();
    };
  ...
  Dispatcher2 = (function() {
    function Dispatcher2() {
      this._item_que = [];
    }

    Dispatcher2.prototype.available_items = function(uid) {
      var fid, item, _i, _ref;
      _ref = this._item_que;
      for (_i = _ref.length - 1; _i >= 0; _i += -1) {
        fid = _ref[_i];
        item = (function() {
        ...
        if (item && item.dispatchable() && !item.has_group_perm(uid)) {
          return true;
        }
      }
      return false;
    ...
    Dispatcher2.prototype.request_item = function(u) {
      var candidates, fid, item, n_candi, reusable, selected, _i, _now, _ref, _results;
      candidates = [];
      n_candi = 0;
      reusable = [];
      _now = now();
      while (this._item_que.length > 0 && n_candi < 30) {
        fid = this._item_que.shift();
        item = (function() {
          try {
            return gDB.feeling(fid);
          } catch (_error) {}
        ...
        reusable.push(fid);
        if (item.has_group_perm(u.id)) {
          continue;
        }
        console.log("item has no perm");
        (function() {
          _results = [];
          for (var _i = 0, _ref = item.weight(_now - item.time); 0 <= _ref ? _i <= _ref : _i >= _ref; 0 <= _ref ? _i++ : _i--){ _results.push(_i); }
          return _results;
        }).apply(this).forEach(function() {
          return candidates.push(item);
        });
        n_candi++;
      }
      selected = candidates.length === 0 ? null : candidates[rand(0, candidates.length - 1)];
      while (reusable.length > 0) {
        ...
        this._item_que.unshift(fid);
      }
      if (selected) {
        this._item_que.push(selected.id);
      }
      return selected;
    };

    Dispatcher2.prototype.register_item = function(id) {
      return this._item_que.push(id);
    };

    Dispatcher2.prototype.latest_feelings = function(n) {
      var fid, r, _i, _ref;
      r = [];
      _ref = this._item_que;
      for (_i = _ref.length - 1; _i >= 0; _i += -1) {
    ...
    Dispatcher2.prototype.log = function() {
      var u, uid, _ref;
      console.log("name, hearts, arrived, my, rcv");
      _ref = gDB.users();
      for (uid in _ref) {
  ...
  app.get('/api/feelings/has_new', function(req, res) {
    var me;
    me = gDB.user(req.session.user_id);
    return res.json({
  ...
  app.get('/api/feelings/new', function(req, res) {
    var f, me;
    me = gDB.user(req.session.user_id);
    if (!me.has_available_feeling()) {
      throw "not ready to get new one";
    }
    f = gDispatcher.request_item(me);
    if (!f) {
      throw "no appropriate item";
    }
    f.grant_group_perm(me.id);
    me.grab_feeling(f.id);
    return res.json(f.extend(me.id));
  });
